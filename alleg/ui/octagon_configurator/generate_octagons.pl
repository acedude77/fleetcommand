...
my $sth=$dbh->prepare('select sector_name,available_techs,treasures,total_cash,resources,starting_cash,map from afcoc');

$sth->execute();

while(($sector_name,$available_techs,$treasures,$total_cash,$resources,$starting_cash,$map)=$sth->fetchrow_array()){
