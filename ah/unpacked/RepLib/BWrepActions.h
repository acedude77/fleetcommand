...
{
public:
	virtual short Convert(short unitID, unsigned long time)=0;
};
...
public:
	...
	// parameters as text
	const char *GetParameters(IUnitIDToObjectID *interf = NULL) const;

	// units IDs as text
	const char *GetUnitsID(IUnitIDToObjectID *interf) const;
	...
	static const char *MkUnitID2String(char *buffer, unsigned short unitid, IUnitIDToObjectID *interf, unsigned long time);

	// convert tick to seconds
	static int Tick2Sec(unsigned long tick);

	// convert seconds to tick
	static unsigned long Sec2Tick(int sec);
...
private:
	...
	// ratio to convert ticks to seconds
	static const float m_timeRatio;//=23.xxx
};
