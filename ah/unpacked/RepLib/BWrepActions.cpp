...
const char *BWrepAction##classname::gGetParameters(const BWrepAction *action, const unsigned char *data, int datasize,IUnitIDToObjectID *interf){\
...
// "leftgame" action
IMPLACTION(LeftGame,8)
	if(p->m_how==1)
		strcpy(gszParams,"left");
	else if(p->m_how==6)
		strcpy(gszParams,"dropped");
	else
...
bool BWrepActionList::DecodeActions(const unsigned char *buffer, int cmdSize, bool clear)
{
	...
	{
		// get time
		unsigned long time = CONSUME(unsigned long);
...
{
	if (m_pGetParamText == NULL || m_parent == NULL)
		return "";

	const unsigned char *newData = (const unsigned char*) m_parent->GetAbsAddress(m_dataoff);
	if (newData == NULL)
		return "";

	char *txt = (char*)m_pGetParamText(this,newData,m_datasize,interf);
	if (txt == NULL)
		return "";
