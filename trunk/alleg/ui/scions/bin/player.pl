...
#===========gather data from db
my $sth=$dbh->prepare('select sector_name,location,map from afcoc_scions');
$sth->execute();
$i=2;
...
if($sectorid=~/\d+/ && $backgrounds ne "" && $icons ne ""){
	$sth=$dbh->prepare("update afcoc_scions set backgrounds='$backgrounds', icons='$icons' where id=$sectorid");
	$sth->execute();
	...
	$sth=$dbh->prepare("select backgrounds,icons from afcoc_scions where id='$sectorid'");
	$sth->execute();
